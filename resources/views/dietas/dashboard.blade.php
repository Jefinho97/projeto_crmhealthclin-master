...
<h1>Dietas Cadastradas</h1>  <button type="button" class="btn btn-success" id="add" style="float:right"> cadastrar dieta</button>
</div>
            ...
            <form  id="form" name="form">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" min="0" class="form-control" id="uni_medida" name="uni_medida">
                    </div>
                    <div class="form-group">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-primary" id="save">Salvar Dieta</button>
                </div>
            </form>
        </div>
    </div>
...
$(function(){
    ...
    $(document).on('click', '#save', function(){
        if(form.nome.value == ""){
            toastr.error('Nome da dieta não informada');
            descricao.focus();
            return;
        }
        if(form.uni_medida.value == ""){
        ...
        if(form.venda.value == ""){
            toastr.error('Preço venda não informado');
            descricao.focus();
            return;
        }
        $.ajax({
            data: $("#form").serialize(),
            url: "{{route('dietas.store')}}",
            type:"POST",
            success: function(){
                table.draw();
                toastr.success('Dieta criada com sucesso!');
                $('#modal').modal('hide');
            },
            error: function(){
        ...
        swal.fire({
            ...
            if(result.value){
                $.ajax({
                    ...
                    success: function(){
                        table.draw();
                        toastr.success('Dieta deletada com sucesso!');
                    },
                    error: function(){
                        toastr.error('Algo deu errado, ERRO!');
                    }
